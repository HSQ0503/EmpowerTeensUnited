  ...
  return (
    <>
      ...
      <section className="etu-section" style={{ padding: "96px 56px", background: "#fff" }}>
        ...
        {team.length === 0 ? (
          <p style={{ textAlign: "center", color: A.muted, fontSize: 15 }}>
            Our team is updating soon.
          </p>
        ) : (
          <Stagger style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 48, maxWidth: 1100, margin: "0 auto" }}>
            {team.map((p) => (
              <StaggerItem key={p.id} style={{ textAlign: "center" }}>
                  ...
                  <div
                    style={{
                      ...
                      background: p.photoUrl
                        ...
                        : `linear-gradient(135deg, ${A.cream}, ${A.gold})`,
                      border: `2px solid ${A.gold}`,
                      padding: 6,
                      boxShadow: `0 0 0 1px ${A.rule}`,
                    }}
                  />
                  <h3 style={{ fontFamily: A.fontHead, fontSize: 22, fontWeight: 500, color: A.navy, margin: "24px 0 6px" }}>{p.fullName}</h3>
                  <div style={{ fontSize: 13, letterSpacing: 1.5, textTransform: "uppercase", color: A.muted, fontWeight: 700, marginBottom: 16 }}>{p.roleTitle}</div>
                  {p.bio && (
